...
| [In-the-wild scans (Dec 1-3)](https://www.malware-traffic-analysis.net/2025/12/17/index.html) | Internet background scanning captured alongside the Mirai sample | 5 HTTP requests extracted (including a `wget`-style command-injection probe) with zero behavior findings; used as a false-positive control rather than an infection case | Control sample |
